import { useEffect, useRef, useState } from "react";
import { useChecklist } from "@/contexts/checklist-context";
import { useActivityPanel } from "@/contexts/loan-activity-context";
import { formatActivityTimestamp } from "@/lib/loan-activity";

type GenerationState = "idle" | "generating" | "generated";

function getActivityId(baseId: string, existingIds: Array<number | string>): string {
  if (!existingIds.includes(baseId)) return baseId;
  const refirePrefix = `${baseId}-refire-`;
  const refireCount = existingIds.filter(
    (id) => typeof id === "string" && id.startsWith(refirePrefix),
  ).length;
  return `${refirePrefix}${refireCount + 1}`;
}

function AnimatedEllipsis() {
  const [dots, setDots] = useState(1);

  useEffect(() => {
    const interval = setInterval(() => {
      setDots((prev) => (prev >= 3 ? 1 : prev + 1));
    }, 400);
    return () => clearInterval(interval);
  }, []);

  return <span>{".".repeat(dots)}</span>;
}

function formatTimestamp(): string {
  const now = new Date();
  const month = String(now.getMonth() + 1).padStart(2, "0");
  const day = String(now.getDate()).padStart(2, "0");
  const year = now.getFullYear();
  let hours = now.getHours();
  const ampm = hours >= 12 ? "PM" : "AM";
  hours = hours % 12;
  if (hours === 0) hours = 12;
  const minutes = String(now.getMinutes()).padStart(2, "0");
  return `${month}/${day}/${year} ${String(hours).padStart(2, "0")}:${minutes} ${ampm}`;
}

interface SecondaryAction {
  label: string;
  onClick: () => void;
  testId?: string;
  marksDelivered?: boolean;
}

export function DocumentSection({
  title,
  idleText,
  generatingText,
  generateLabel,
  secondaryActions,
  testIdPrefix,
  onGenerated,
  resetVersion,
}: {
  title: string;
  idleText: string;
  generatingText: string;
  generateLabel: string;
  secondaryActions?: SecondaryAction[];
  testIdPrefix: string;
  onGenerated?: (generatedAt: Date) => void;
  resetVersion?: number;
}) {
  const [generationState, setGenerationState] = useState<GenerationState>("idle");
  const [lastGenerated, setLastGenerated] = useState<string | null>(null);
  const [lastDelivered, setLastDelivered] = useState<string | null>(null);
  const timerRef = useRef<ReturnType<typeof setTimeout> | null>(null);

  useEffect(() => {
    if (timerRef.current) {
      clearTimeout(timerRef.current);
      timerRef.current = null;
    }
    setGenerationState("idle");
    setLastGenerated(null);
    setLastDelivered(null);
  }, [resetVersion]);

  useEffect(() => {
    return () => {
      if (timerRef.current) clearTimeout(timerRef.current);
    };
  }, []);

  const handleGenerate = () => {
    setGenerationState("generating");
    timerRef.current = setTimeout(() => {
      const generatedAt = new Date();
      setLastGenerated(formatTimestamp());
      setGenerationState("generated");
      onGenerated?.(generatedAt);
    }, 5000);
  };

  return (
    <div className="flex flex-col" style={{ gap: "var(--roads-spacing-component-l)" }}>
      <h2
        className="headline-200"
        style={{ color: "var(--roads-text-primary)" }}
        data-testid={`text-${testIdPrefix}-title`}
      >
        {title}
      </h2>

      <div className="flex flex-col" style={{ gap: "var(--roads-spacing-component-l)" }}>
        <p
          className="body-100"
          style={{ color: "var(--roads-text-secondary)" }}
          data-testid={`text-${testIdPrefix}-status`}
        >
          {generationState === "idle" && idleText}
          {generationState === "generating" && (
            <span>
              {generatingText}<AnimatedEllipsis />
            </span>
          )}
          {generationState === "generated" && `Last Generated: ${lastGenerated}`}
        </p>

        {lastDelivered && (
          <p
            className="body-100"
            style={{ color: "var(--roads-text-secondary)" }}
            data-testid={`text-${testIdPrefix}-delivered`}
          >
            {`Marked Delivered: ${lastDelivered}`}
          </p>
        )}

        <div className="flex items-center" style={{ gap: "var(--roads-spacing-component-l)" }}>
          <button
            onClick={() => {
              if (generationState === "generating") return;
              handleGenerate();
            }}
            aria-disabled={generationState === "generating"}
            className="label-strong"
            style={{
              backgroundColor: "var(--roads-bg-action)",
              color: "var(--roads-text-reverse)",
              padding:
                "var(--roads-spacing-component-xs) var(--roads-spacing-component-l)",
              borderRadius: "var(--roads-radius-2xs)",
              border: "none",
              cursor: generationState === "generating" ? "not-allowed" : "pointer",
              whiteSpace: "nowrap",
            }}
            data-testid={`button-generate-${testIdPrefix}`}
          >
            {generateLabel}
          </button>

          {(secondaryActions ?? []).map((action, index) => (
            <button
              key={`${testIdPrefix}-secondary-${index}`}
              onClick={() => {
                if (generationState !== "generated") return;
                action.onClick();
                if (action.marksDelivered) {
                  setLastDelivered(formatTimestamp());
                }
              }}
              aria-disabled={generationState !== "generated"}
              className="label-strong"
              style={{
                backgroundColor: "var(--roads-bg-primary)",
                color: "var(--roads-text-primary)",
                padding:
                  "var(--roads-spacing-component-xs) var(--roads-spacing-component-l)",
                borderRadius: "var(--roads-radius-2xs)",
                border: "1px solid var(--roads-border-dark)",
                cursor: generationState !== "generated" ? "not-allowed" : "pointer",
                whiteSpace: "nowrap",
              }}
              data-testid={
                action.testId ??
                (index === 0
                  ? `button-secondary-${testIdPrefix}`
                  : `button-secondary-${testIdPrefix}-${index}`)
              }
            >
              {action.label}
            </button>
          ))}
        </div>
      </div>
    </div>
  );
}

export function DocumentsContent() {
  const { completeAction, getActionStatus, demoResetVersion } = useChecklist();
  const { addActivity, activityItems } = useActivityPanel();

  const handleEarlyDisclosuresGenerated = (generatedAt: Date) => {
    if (getActionStatus("Early Disclosures", 0) === "complete") return;

    completeAction("Early Disclosures", 0);
    addActivity({
      id: "early-disclosures-generated",
      title: "Document Generated",
      description: "Early Disclosures generated",
      timestamp: formatActivityTimestamp(generatedAt),
      date: generatedAt,
    });
  };

  const handleEarlyDisclosuresDelivered = () => {
    if (getActionStatus("Early Disclosures", 1) === "complete") return;

    const deliveredAt = new Date();
    completeAction("Early Disclosures", 1);
    addActivity({
      id: "early-disclosures-delivered",
      title: "Document Delivered",
      description: "Early Disclosures delivered",
      timestamp: formatActivityTimestamp(deliveredAt),
      date: deliveredAt,
    });
  };

  const handleLoanEstimateGenerated = (generatedAt: Date) => {
    const actionStatus = getActionStatus("Loan Estimate", 0);
    if (actionStatus === "complete") return;
    const isRepeat = activityItems.some((item) => item.id === "loan-estimate-generated");

    completeAction("Loan Estimate", 0);
    addActivity({
      id: isRepeat
        ? getActivityId(
            "loan-estimate-generated",
            activityItems.map((item) => item.id),
          )
        : "loan-estimate-generated",
      title: "Document Generated",
      description: "Loan Estimate generated",
      timestamp: formatActivityTimestamp(generatedAt),
      date: generatedAt,
    });
  };

  const handleLoanEstimateDelivered = () => {
    if (getActionStatus("Loan Estimate", 1) === "complete") return;

    const deliveredAt = new Date();
    const isRepeat = activityItems.some((item) => item.id === "loan-estimate-delivered");
    completeAction("Loan Estimate", 1);
    addActivity({
      id: isRepeat
        ? getActivityId(
            "loan-estimate-delivered",
            activityItems.map((item) => item.id),
          )
        : "loan-estimate-delivered",
      title: "Document Delivered",
      description: "Loan Estimate delivered",
      timestamp: formatActivityTimestamp(deliveredAt),
      date: deliveredAt,
    });
  };

  return (
    <div
      className="flex flex-col"
      style={{
        padding: "var(--roads-spacing-component-xl)",
        gap: "var(--roads-spacing-component-xl)",
      }}
      data-testid="disclosures-content"
    >
      <DocumentSection
        title="At-App Disclosures"
        idleText="No At-App Disclosures have been generated"
        generatingText="Generating At-App Disclosures"
        generateLabel="Generate At-App Disclosures"
        secondaryActions={[
          {
            label: "Download At-App Disclosures",
            onClick: () => window.open("/at-app-disclosures.pdf", "_blank"),
          },
          {
            label: "Deliver At-App Disclosures",
            onClick: handleEarlyDisclosuresDelivered,
            marksDelivered: true,
          },
        ]}
        testIdPrefix="at-app-disclosures"
        onGenerated={handleEarlyDisclosuresGenerated}
        resetVersion={demoResetVersion}
      />

      <DocumentSection
        title="Loan Estimate"
        idleText="No Loan Estimates have been generated"
        generatingText="Generating Loan Estimate"
        generateLabel="Generate Loan Estimate"
        secondaryActions={[
          {
            label: "View Loan Estimate",
            onClick: () => window.open("/loan-estimate.pdf", "_blank"),
          },
          {
            label: "Deliver Loan Estimate",
            onClick: handleLoanEstimateDelivered,
            marksDelivered: true,
          },
        ]}
        testIdPrefix="loan-estimate"
        onGenerated={handleLoanEstimateGenerated}
        resetVersion={demoResetVersion}
      />
    </div>
  );
}